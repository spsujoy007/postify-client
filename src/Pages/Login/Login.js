...
const Login = () => {
    const {user, loginemail, signInwithPop} = useContext(AuthContext);
	...
	<form onSubmit={handleLogin} className="space-y-6 ng-untouched ng-pristine ng-valid">
		...
		<button type='submit' className="block btn w-full p-3 text-center rounded-sm text-gray-50 bg-primary">Login</button>
	</form>
	...
	<p className="text-lg text-center text-gray-600">Don't have an account?
		<Link rel="noopener noreferrer" className="underline text-gray-800" to="/signup">Signup</Link>
	</p>
</div>
        </div>
        </div>
